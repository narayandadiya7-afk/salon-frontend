...
type LucideIcon,
  ...
  PanelLeftClose, PanelLeft, ChevronDown, ChevronLeft, Menu, Plus, X, Sparkles, LogOut, UserRound,
} from "lucide-react";
      ...
      <div className={cn("flex h-16 shrink-0 items-center gap-2.5 border-b border-sidebar-border px-4", collapsed && "justify-center px-0")}>
        ...
        {!collapsed && (
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-sidebar-accent-foreground">SalonOS</p>
            <p className="truncate text-[0.7rem] uppercase tracking-[0.16em] text-sidebar-foreground/50">Super Admin</p>
          </div>
        )}
      </div>
      ...
      <nav className="flex flex-1 flex-col gap-6 overflow-y-auto px-3 py-4" aria-label="Main">
        ...
        {!collapsed && (
          <div className="mt-auto rounded-xl border border-sidebar-border bg-sidebar-accent/40 p-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-sidebar-primary">Platform status</p>
            <p className="mt-1 flex items-center gap-1.5 text-sm text-sidebar-foreground">
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <aside className="absolute inset-y-0 left-0 w-72 border-r border-sidebar-border bg-sidebar">
              ...
              <SidebarContent collapsed={false} onNavigate={() => setMobileOpen(false)} onToggle={() => setMobileOpen(false)} />
            </aside>
          </div>
        )}
        ...
        <div className={cn("transition-[padding] duration-300", collapsed ? "lg:pl-[76px]" : "lg:pl-[268px]")}>
          <header className="sticky top-0 z-30 border-b border-border bg-background/85 backdrop-blur-xl">
            ...
            <div className="flex min-w-0 items-center gap-2">
              ...
              <Button
                variant="ghost" size="icon" className="-ml-3 hidden lg:inline-flex"
                aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="secondary" className="hidden h-9 gap-2 md:inline-flex">
          ...
          <main className="mx-auto max-w-[1500px] px-4 pb-16 pt-6 sm:px-6 lg:px-8">{children}</main>
        </div>
      </div>
