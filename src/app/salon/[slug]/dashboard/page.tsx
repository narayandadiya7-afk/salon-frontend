...
function DashboardContent() {
  ...
  return (
    <div>
      {isExpired && (
        <Alert type="error" message={<Space><ExclamationCircleOutlined /><span><strong>Subscription Expired</strong> — Renew to reactivate your salon website.</span></Space>}
      ...
      {!isExpired && daysLeft <= 7 && daysLeft > 0 && (
        <Alert type="warning" message={<Space><ClockCircleOutlined /><span>Subscription expires in <strong>{daysLeft} days</strong></span></Space>}
        ...
        <Col xs={24} lg={8}>
          <Card className="premium-card" title={<Space><UserOutlined style={{ color: '#C9953F' }} /><span>Staff Performance</span></Space>}>
            ...
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--theme-border-light)' }}>
              <Link href={`/salon/${slug}/team`}><Button type="link" style={{ padding: 0, fontSize: 13, color: '#7C1D3E' }}>View All Staff <RightOutlined /></Button></Link>
            </div>
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={[20, 20]} style={{ marginBottom: 24 }}>
        <Col xs={24} lg={8}>
          <Card className="premium-card" title={<Space><ScissorOutlined style={{ color: '#C9953F' }} /><span>Popular Services</span></Space>}
            extra={<Link href={`/salon/${slug}/services`}><Button type="link" style={{ fontSize: 12, color: '#7C1D3E' }}>View All</Button></Link>}>
